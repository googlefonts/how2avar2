      ...
      <div className="prose dark:prose-invert mt-16 min-w-0 flex-1">
        ...
        {groups.map(([groupName, { htmls, pngs, mds }]) => (
          <Fragment key={groupName}>
            <h2 id={groupName} className="scroll-mt-12">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-1">
                      ...
                      />
                      <details
                        id={`details-${groupName}-${htmlName}`}
                        open={defaultOpen}
                      >
                        <summary className="cursor-pointer">{htmlName}</summary>
                          ...
                          <ul>
                            {screenshots.map((png) => {
                              ...
                              const failMdPath = mds.includes(failMd)
                                ? `${groupName}/${failMd}`
                                : null;
                              const status = screenshotStatusOf(
                                pngName,
                                failMdPath,
                              );
