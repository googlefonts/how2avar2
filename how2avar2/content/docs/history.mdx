...
Type design originated from calligraphy. Unlike type designers, calligraphers have full control over the size, spacing, and shape of every letter. Although calligraphers draw each letter with slight differences, they have full control over the entire layout and design. Even today, OpenType fonts can only try to mimic the art of calligraphy and lettering styles.
...
For centuries[^fn5], type designers worked with punch cutters to create metal type. The design of metal type mimicked calligraphy to ease the transition to printing, while still being familiar to readers. Because the craftsmen created each metal letter in a specific size, weight, and style, they fine-tuned each letter for its particular use. Punch cutters often improved type designers’ original drawings for small sizes and printing.
...
<figure>
  ...
  />
  <figcaption>
    A photo of the Akzidenz-Grotesk disc used in phototypesetting,
    [provided](https://blog.sbb.berlin/filmreihe/) by Dr. Christian Mathieu,
    Kerstin Wallbach, Dr. Dan Reynolds and the Berlin State Library.
  </figcaption>
</figure>
...
<figure>
  <Image
    src={notFound}
    alt="Screenshot of the Ikarus system, which still works on UNIX"
  />
  <figcaption>
    Screenshot of the Ikarus system,
    [provided](https://www.fontmaster.nl/history.html) by the Dutch Type
    Library.
  </figcaption>
</figure>
...
Ikarus was the first industry standard for digital font data, based on curves instead of pixels. In addition, it could interpolate between light and bold letters with the same number of points. Ikarus could also use “antiquing[^fn20]” to add effects like shadows, outlines, and distortions.
